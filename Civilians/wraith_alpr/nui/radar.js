...
$(function() {
	...
	var radarContainerPlate = $("#platereader");
	...
	window.addEventListener("message", function(event) {
		...
		if (item.toggleradar) {
            ...
            radarContainerPlate.fadeToggle();
		}

		if (item.hideradar) {
			radarContainer.fadeOut();
            radarContainerPlate.fadeOut();
		} else if (item.hideradar == false) {
			radarContainer.fadeIn();
            radarContainerPlate.fadeIn();
		}

		if (item.fwdPlate) {
			updatePlate("fwdPlate", item.fwdPlate);
		}
...
function sendData(name, data) {
	$.post("http://" + resourceName + "/" + name, JSON.stringify(data), function(
		datab
	) {
		if (datab != "ok") {
			console.log(datab);
