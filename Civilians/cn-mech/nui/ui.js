...
$( function() {
    ...
    window.addEventListener( 'message', function( event ) {
        ...
        if(item.mod) {
            mod = item.mod;
            lvl = item.lvl+2;
            $("#"+mod+" .current-mod").remove();
            $("#"+mod+">button[data-action='"+mod+lvl+"']").html($("#"+mod+">button[data-action='"+mod+lvl+"']").text()+' <span class="current-mod">✓</span>');
        }
    } );
    ...
    $( ".menuoption" ).each( function( i, obj ) {
        ...
        if ( $( this ).attr( "data-sub" ) ) {
            ...
            $( this ).click( function() {
                element.show();
                $( this ).parent().hide();
                if(ToSend.includes(menu)) {
                    sendData( "ButtonClick", menu );
                }
            } )
            ...
            // submenu. 
            var backBtn = $( '<button/>', { text: 'Tilbage' } );
...
function sendData( name, data ) {
    $.post( "http://cn-mech/" + name, JSON.stringify( data ), function( datab ) {
        if ( datab != "ok" ) {
            console.log( datab );
        }            
    } );
