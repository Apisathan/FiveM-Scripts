...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if(item.updateBalance == true) {
            $('.currentBalance').html(addCommas(event.data.balance +' DKK'));
            $('.currentDebt').html(addCommas(event.data.debt +' DKK'));
            $('.username').html(event.data.player);
        }
        // Trigger Add Balance Popup
        if(item.addBalance == true) {
            var element = $('<p id="add-balance"><span class="pre">+</span><span class="green"> DKK </span>' +addGaps(event.data.amount)+'</p>');
        ...
        if(item.removeBalance == true) {
            var element = $('<p id="add-balance"><span class="pre">-</span><span class="red"> DKK </span>' +addGaps(event.data.amount)+'</p>');
            $(".transaction").append(element);
            setTimeout(function(){
                $(element).fadeOut(600, function() { $(this).remove(); })
            }, 1000)
    ...
    $(".btnInsertAll").click(function(){
        $.post('http://banking/addAll', JSON.stringify({}));
    });
